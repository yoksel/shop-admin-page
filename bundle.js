...
async function renderPage (path, match) {
  const publicUrl = process.env.PUBLIC_URL || 'https://yoksel.github.io/shop-admin-page/';

  const { default: Page } = await __webpack_require__("./src/lib lazy recursive ^.*\\/pages\\/.*\\/index\\.js$")(`${publicUrl}/pages/${path}/index.js`);
